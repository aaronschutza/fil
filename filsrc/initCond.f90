...
subroutine initialConditions1
	...
	!********** sorry for the mess I'll be done in a second... ********
	...
	if(plotPVG)then
		...
		do k = 1,PVGgrid
			...
			if(rcmdata .or. useAnalyticModelGrid)then
				...
				x0 = apexi(k)
				do i = 1,plasGridi
					call line_intersects(x0,zg(1),x0,zg(gridj),plasGridx(i),plasGridz(i),plasGridx(i+1),plasGridz(i+1),isopen,x1,z0,1,i,.true.)
					if (isopen) then
						write(*,*) x0,z0
						exit
					endif
	...
	if(plotPVG)then
		...
		do i = 2,PVGgrid-1
			write(11,*) apexi(i),apex_z(i),midPres(i),midBz(i),midV(i),midN(i),PVG_bg(i),(PVG_bg(i+1)-PVG_bg(i-1))/(apexi(i+1)-apexi(i-1)), &
...
subroutine loadFilament
	...
	open(unit=111, file=trim(datapath)//".ext", status="old")
	read(111,*) in1
	if (in1/=dimi) then
		write(*,*) 'error: dimi mismatch while reading .ext file'
		write(*,*) 'dimi=',dimi
		write(*,*) 'input=',in1
		stop
	endif
	read(111,*) in2
	if (in2/=dimj) then
		write(*,*) 'error: dimj mismatch while reading .ext file'
		write(*,*) 'dimj=',dimj
		write(*,*) 'input=',in2
		stop
	endif
	...
	close(111)
end subroutine loadFilament

subroutine saveFilament
	use stateMod
	use stateIndex
	use initCond
	use parametersBackground
	...
	open(unit=111, file=trim(datapath)//".ext", status="replace")
	close(111)
