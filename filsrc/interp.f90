...
real function bilinearInterp(mesh,x,z,elnum)
    ...
    character(len=*) :: mesh
       ...
       if(x>xmaxl .or. z>zmaxl .or. x<xminl .or. z<zminl)then
              ...
              write(*,*)'xmin=',xminl,' x=',x,' xmax=',xmaxl
              write(*,*)'zmin=',zminl,' z=',z,' zmax=',zmaxl
              stop
       else
!********************************************************************************
...
!***************************************************************************************
       ...
       select case(mesh)
              ...
              case('dx_Pg')
                     coef1 = (xg(i+1)-xg(i))*(zg(j+1)-zg(j))
                     bilinearInterp = dx_Pg(i,j)*(xg(i+1)-x)*(zg(j+1)-z)/coef1 &
                                          & +  dx_Pg(i+1,j)*(x-xg(i))*(zg(j+1)-z)/coef1 &
                                          & +  dx_Pg(i,j+1)*(xg(i+1)-x)*(z-zg(j))/coef1 &
                                          & +  dx_Pg(i+1,j+1)*(x-xg(i))*(z-zg(j))/coef1
              case('dz_Pg')
                     coef1 = (xg(i+1)-xg(i))*(zg(j+1)-zg(j))
                     bilinearInterp = dz_Pg(i,j)*(xg(i+1)-x)*(zg(j+1)-z)/coef1 &
                                          & +  dz_Pg(i+1,j)*(x-xg(i))*(zg(j+1)-z)/coef1 &
                                          & +  dz_Pg(i,j+1)*(xg(i+1)-x)*(z-zg(j))/coef1 &
                                          & +  dz_Pg(i+1,j+1)*(x-xg(i))*(z-zg(j))/coef1
              case('testMesh')
                     coef1 = (xg(i+1)-xg(i))*(zg(j+1)-zg(j))
